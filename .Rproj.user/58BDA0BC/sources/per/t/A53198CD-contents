...
dpcca.mod = "model
{
  ...
  for (s in 2:t) {
    ...
    for (i in 1:n) {
      mu[s,i] =  a + b*(s/t)
      Z[s, i] = rnorm(1, phi*Z[s -1 ,i] + mu[s,i], 3)

    }


    X1[s, , p.sel[1]] = w1[1]*Z[s,] #+ rnorm(n, 0, 1)
    X1[s, , p.sel[2]] = w1[2]*Z[s,] #+ rnorm(n, 0, 1)
    X2[s, , q.sel[1]] = w2[1]*Z[s,] #+ rnorm(n, 0, 1)
  }
  ...
  mod = run.jags(data = data, model=dpcca.mod, monitor=c("phi", "b", "w1", "w2", "S1", "S2"), n.chains=3,
                 method="rjparallel", burnin = 1000, sample = 2000, adapt = 100)
  ...
  ind_param = c(1,2, p.sel+2, p +q.sel+2) # -((p+q):(p+q+2))
  z_param   = sample((1:(p+q+2))[-ind_param], 1)
  ind_param[length(ind_param) + 1] <- z_param
  true_param = abs(c(phi, b,  w1, w2, 0))

  bb1 = (mod$mcmc[[1]][,ind_param])

  bb2 = (mod$mcmc[[2]][,ind_param])
  bb3 = (mod$mcmc[[3]][,ind_param])
  ...
  X = (apply(X1,3,"c"))
  # MS to NS
  X1 = array(sapply(alpha, function(a) rnorm(length(Z), Z*a, 1)), c(length(t), n, p))
  X2 = array(sapply(beta,  function(b) rnorm(length(Z), Z*b, 1)), c(length(t), n, q))
