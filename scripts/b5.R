...
for (s in 2:t) {
  ...
  for (i in 1:n) {
    mu[s,i] =  a + b*(s/t)
    Z[s, i] = phi*Z[s -1 ,i] + mu[s,i] + eps[1,i]
...
dpcca.mod = "model
{
  ...
  # results ----------------------------------------------------------------------
  mod = run.jags(data = data, model=dpcca.mod, monitor=c("b", "phi", "w1", "w2", "S1", "S2"), n.chains=3,
  ...
  true_param = abs(c(b, phi, w1,w2, 0))
...

  {
    ...
    plot(as.matrix(bb1[,i]), type="p", main=colnames(bb1)[i], ylim = c(min(bb1[,i], bb2[,i], bb3[,i], true_param[i]), max(bb1[,i], bb2[,i], bb3[,i], true_param[i])))
